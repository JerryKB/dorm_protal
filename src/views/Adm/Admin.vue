<template>
    ...
    <div>
        <div style="height: 60px;margin-bottom: 5px;padding-top: 0px;line-height: 48px">
            ...
            <el-button type="primary" class="left" ref="addBtn" @click="dialogFormVisibleAdd=true">新增管理员</el-button>
        </div>
        <el-table
                class="el-main"
                :data="tableData"
...
<script>
    ...
    export default {
        ...
        methods: {
            initTableData(){
                this.getRequest("/admin/").then(data=>{
                    this.tableData=data;
                })
            },
            deleteById(index,data){
                this.$confirm('此操作将永久删除'+ data.id+'号宿舍楼数据, 是否继续?', '提示', {
                    ...
                    this.deleteRequest("/admin/"+data.id).then(data=>{
                    })
                    this.initTableData();
                }).catch(() => {
                    this.$message({
            ...
            editFinish(){
                this.putRequest("/admin/",this.form).then(data=>{
            ...
            addFinish(){
                this.postRequest("/admin/",this.addForm).then(resp=>{
